...
# ---------------------------------------------------------------------------
# POST /multiDocQuery — query across multiple (or all) indexed documents
# ---------------------------------------------------------------------------

@app.route(route="multiDocQuery", methods=["POST"])
def multi_doc_query(req: func.HttpRequest) -> func.HttpResponse:
    logging.info("POST /multiDocQuery")
    try:
        body = req.get_json()
    except Exception:
        return func.HttpResponse(
    ...
    doc_ids     = body.get("documentIds") or []   # optional list of doc_id strings
    ...
    try:
        ...
        # 2. Search — optionally filter to specific doc_ids
        ...
        # 3. If caller specified doc_ids, filter chunks to those docs only
        if doc_ids:
            doc_id_set = set(doc_ids)
            chunks = [c for c in chunks if c.get("doc_id") in doc_id_set]

        if not chunks:
            return func.HttpResponse(
                json.dumps({
                    "answer":  "No relevant content found across the selected documents.",
                    "sources": [],
                }),
                status_code=200, mimetype="application/json")

        # 4. Build context from chunks, tracking unique sources
        context_parts = []
        seen_docs     = {}   # doc_id → filename
        for c in chunks:
            context_parts.append(f"[{c['filename']}]\n{c['text']}")
            seen_docs[c["doc_id"]] = c["filename"]

        context = "\n\n---\n\n".join(context_parts)

        # 5. Generate combined answer via LLM
        system_prompt = (
            "You are a helpful assistant. Answer the user's question using ONLY the "
            "provided document excerpts. Cite which document(s) each piece of information "
            "comes from. If the answer spans multiple documents, synthesise them clearly."
        )
        ...
        sources = [
            {"documentId": doc_id, "fileName": fname}
        ...
        logging.exception("multiDocQuery error.")
        return func.HttpResponse(
            json.dumps({"error": "Internal server error.", "detail": str(exc)}),
            status_code=500, mimetype="application/json")
